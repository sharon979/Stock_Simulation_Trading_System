...
<head>
    ...
    <link rel="stylesheet" href="fails.css" charset="utf-8">
</head>
<body>
    <div class="warp">
        <div class="title">
            ...
            <div class="table1">
                <?php
                    $link = @mysqli_connect( 
                        ...
                        "sharonwang",  // 密碼 
                        "stockproject");  // 預設使用的資料庫名稱 
                        $sql="SELECT * FROM fails WHERE account='$nowaccount' ";
                        $result = mysqli_query($link, $sql);
                        if (!$result) {
                           printf("Error: %s\n", mysqli_error($link));
                           exit();
                        ...
                        echo"<tr bgcolor='#f5f290'>"."<td>"."股票編號"."</td>"."<td>"."張數"."</td>"."<td>"."失敗原因"."</td>";
                       if ( $result = mysqli_query($link, $sql) ) {
                            while($row = mysqli_fetch_assoc($result)){  
                        echo"<tr bgcolor=#e0e0e0 style='font-size:18px;border:1px #f0f0f0 solid;border-collapse:collapse;' rules='all' cellpadding='10px';>"."<td>".$row["stocknum"]."</td>"."<td>".$row["howmany"]."</td>"."<td>".$row["why"]."</td>"."</br>";                        
                            }
                        }
                ?>
            </div>
